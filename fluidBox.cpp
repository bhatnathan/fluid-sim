...
FluidBox::FluidBox() : size(1), diffusion(0), viscosity(0.1) {
...
FluidBox::FluidBox(int size, float diffusion, float viscosity) : size(size), diffusion(diffusion), viscosity(viscosity) {
	this->transform = Transform();
		...
		for (int k = 0; k < size; k++) {
			for (int j = 0; j < size; j++) {
				for (int i = 0; i < size; i++) {
					...
					transform.setMatrix();
					glTranslatef(i - size / 2, j - size / 2, k - size / 2);
	...
	for (int i = -1; i < 2; i++) {
		for (int j = -1; j < 2; j++) {
			for (int k = -1; k < 2; k++) {
				...
				addDensity(ivec3(size / 2 + i, size / 2 + j, size / 2 + k), r / 5);
				...
				addVelocity(ivec3(size / 2 + i, size / 2 + j, size / 2 + k), vec3(0, -1, 0));
			}
		}
	}
...
void FluidBox::set_bnd(int b, float* x) {
	...
	x[IX(0, 0, 0)] = 0.33f * (x[IX(1, 0, 0)]
		+ x[IX(0, 1, 0)]
		+ x[IX(0, 0, 1)]);
	x[IX(0, size - 1, 0)] = 0.33f * (x[IX(1, size - 1, 0)]
		+ x[IX(0, size - 2, 0)]
		+ x[IX(0, size - 1, 1)]);
	x[IX(0, 0, size - 1)] = 0.33f * (x[IX(1, 0, size - 1)]
		+ x[IX(0, 1, size - 1)]
		+ x[IX(0, 0, size - 2)]); //changed from size to size -2 as I think this was a mistake
	x[IX(0, size - 1, size - 1)] = 0.33f * (x[IX(1, size - 1, size - 1)]
		+ x[IX(0, size - 2, size - 1)]
		+ x[IX(0, size - 1, size - 2)]);
	x[IX(size - 1, 0, 0)] = 0.33f * (x[IX(size - 2, 0, 0)]
		+ x[IX(size - 1, 1, 0)]
		+ x[IX(size - 1, 0, 1)]);
	x[IX(size - 1, size - 1, 0)] = 0.33f * (x[IX(size - 2, size - 1, 0)]
		+ x[IX(size - 1, size - 2, 0)]
		+ x[IX(size - 1, size - 1, 1)]);
	x[IX(size - 1, 0, size - 1)] = 0.33f * (x[IX(size - 2, 0, size - 1)]
		+ x[IX(size - 1, 1, size - 1)]
		+ x[IX(size - 1, 0, size - 2)]);
	x[IX(size - 1, size - 1, size - 1)] = 0.33f * (x[IX(size - 2, size - 1, size - 1)]
		+ x[IX(size - 1, size - 2, size - 1)]
		+ x[IX(size - 1, size - 1, size - 2)]);
}
	...
	for (k = 1, kfloat = 1; k < size - 1; k++, kfloat++) {
		for (j = 1, jfloat = 1; j < size - 1; j++, jfloat++) {
			for (i = 1, ifloat = 1; i < size - 1; i++, ifloat++) {
				...
				i0i = std::max(0, std::min(size - 1, i0i));
				i1i = std::max(0, std::min(size - 1, i1i));
				j0i = std::max(0, std::min(size - 1, j0i));
				j1i = std::max(0, std::min(size - 1, j1i));
				k0i = std::max(0, std::min(size - 1, k0i));
				k1i = std::max(0, std::min(size - 1, k1i));
